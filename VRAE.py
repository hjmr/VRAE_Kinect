...
class VRAE(nn.Module):
    ...
    def encode(self, enc_inp):
        n_batch = len(enc_inp)
        inp, _ = make_padded_sequence(enc_inp)
        _, (h_enc, _) = self.encoder(inp.to(self.get_device()))
    ...
    def decode(self, z, dec_inp):
        ...
        inp, seq_len = make_padded_sequence(dec_inp)
        out, _ = self.decoder(inp.to(self.get_device()),
                              (h_dec.view(self.n_dec_layers, n_batch, self.n_dec_hidden), dec_c0))
        dec_out = self.linear_out(out)
        return make_unpadded_sequence(dec_out, seq_len)

    def generate(self, z, seq_len):
        with torch.no_grad():
            dec_inp = make_ones(seq_len, self.n_input)
    ...
    def loss(self, enc_inp, beta=1.0, k=1):
        ...
        dec_inp = make_ones(inp_len, self.n_input)
